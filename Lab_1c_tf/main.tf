...
ingress {
    ...
    to_port     = 3306
    protocol    = "tcp"
    security_groups = [aws_security_group.dawgs_armageddon_ec2_sg01.id]
  }
...
resource "aws_ssm_parameter" "dawgs_armageddon_endpoint_param" {
...
# Explanation: If secret with name aready exists and is scheduled for deletion, Secrets Manager locks the name until the deletion window expires.
# I choose to reuse the same name and will run the below in bas:
#  aws secretsmanager restore-secret --secret-id dawgs_armageddon/rds/mysql

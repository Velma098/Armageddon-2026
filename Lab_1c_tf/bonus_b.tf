...
locals {
  ...
  dawgs_armageddon_fqdn = "${var.app_subdomain}.${var.domain_name}"
}
...
resource "aws_security_group" "dawgs_armageddon_alb_sg01" {
...
resource "aws_security_group_rule" "dawgs_armageddon_ec2_ingress_from_alb01" {
  type                     = "ingress"
  security_group_id        = aws_security_group.dawgs_armageddon_ec2_sg01.id
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.dawgs_armageddon_alb_sg01.id
...
resource "aws_lb" "dawgs_armageddon_alb01" {
  name               = "${var.project_name}-alb01"
  load_balancer_type = "application"
  internal           = false

  security_groups = [aws_security_group.dawgs_armageddon_alb_sg01.id]
  subnets         = aws_subnet.dawgs_armageddon_public_subnets[*].id
...
resource "aws_lb_target_group" "dawgs_armageddon_tg01" {
...
resource "aws_lb_target_group_attachment" "dawgs_armageddon_tg_attach01" {
  target_group_arn = aws_lb_target_group.dawgs_armageddon_tg01.arn
  target_id        = aws_instance.dawgs_armageddon_ec201_private_bonus.id
  port             = 80
...
resource "aws_acm_certificate" "dawgs_armageddon_acm_cert01" {
  domain_name       = local.dawgs_armageddon_fqdn
...
resource "aws_acm_certificate_validation" "dawgs_armageddon_acm_validation01" {
  certificate_arn = aws_acm_certificate.dawgs_armageddon_acm_cert01.arn
...
resource "aws_lb_listener" "dawgs_armageddon_http_listener01" {
  load_balancer_arn = aws_lb.dawgs_armageddon_alb01.arn
  port              = 80
  protocol          = "HTTP"
...
resource "aws_lb_listener" "dawgs_armageddon_https_listener01" {
  load_balancer_arn = aws_lb.dawgs_armageddon_alb01.arn
  port              = 443
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-TLS13-1-2-2021-06"
  certificate_arn   = aws_acm_certificate_validation.dawgs_armageddon_acm_validation01.certificate_arn
  ...
  depends_on = [aws_acm_certificate_validation.dawgs_armageddon_acm_validation01]
}
...
resource "aws_wafv2_web_acl" "dawgs_armageddon_waf01" {
...
resource "aws_wafv2_web_acl_association" "dawgs_armageddon_waf_assoc01" {
  count = var.enable_waf ? 1 : 0

  resource_arn = aws_lb.dawgs_armageddon_alb01.arn
  web_acl_arn  = aws_wafv2_web_acl.dawgs_armageddon_waf01[0].arn
}
...
resource "aws_cloudwatch_metric_alarm" "dawgs_armageddon_alb_5xx_alarm01" {
  ...
  dimensions = {
    LoadBalancer = aws_lb.dawgs_armageddon_alb01.arn_suffix
  }
...
resource "aws_cloudwatch_dashboard" "dawgs_armageddon_dashboard01" {
    ...
    widgets = [
      {
        ...
        properties = {
          metrics = [
            [ "AWS/ApplicationELB", "RequestCount", "LoadBalancer", aws_lb.dawgs_armageddon_alb01.arn_suffix ],
            [ ".", "HTTPCode_ELB_5XX_Count", ".", aws_lb.dawgs_armageddon_alb01.arn_suffix ]
          ]
          period = 300
          stat   = "Sum"
          region = var.aws_region
          title  = "dawgs_armageddon ALB: Requests + 5XX"
        }
      },
      {
        type  = "metric"
        ...
        properties = {
          metrics = [
            [ "AWS/ApplicationELB", "TargetResponseTime", "LoadBalancer", aws_lb.dawgs_armageddon_alb01.arn_suffix ]
          ]
          period = 300
          stat   = "Average"
          region = var.aws_region
          title  = "dawgs_armageddon ALB: Target Response Time"
        }
      }
    ]
  })
